...
(
    [hdn_id_cidade] => 8105
    [hdn_id_sol_funcionamento] => 126
    [hdn_id_tp_sol_funcionamento] => S
    [hdn_id_vist_func] => 140
    [txt_id_prot_funcionamento] => 100
    [txt_nr_cnpj_empresa] => 04.094.401/0001-09
    [txt_nm_razao_social] => POSTO DE SERVIÇOS ILHA BELA LTDA
    [txt_nm_fantasia_empresa] => POSTO ILHA BELA
    [txt_nm_contato] => ENILTON ERIDES OLIVEIRA
    [txt_nr_fone_empresa] => 32408357
    [txt_de_email_empresa] => naotememail@cb.com.br
    [txt_id_edificacao] => 328
    [txt_nm_edificacao] => POSTO DE SERVIÇOS ILHA BELA
    [hdn_id_tp_logradouro] => 
    [txt_nm_tp_logradouro] => RODOVIA
    [txt_nm_logradouro] => ADMAR GONZAGA
    [txt_nr_edificacao] => 2.565
    [txt_nm_bairro] => ITACORUBI
    [txt_id_cep] => 88.034-000
    [txt_nm_cidade] => FLORIANOPOLIS
    [txt_nm_complemento] => 12.04.09           PRÓXIMO A FIESC
    [txt_vl_area_construida] => 764,36
    [txt_vl_area_vistoriada] => 4,00
    [cmb_ch_tp_funcionamento] => P
    [cmb_desc_funcionamento] => 
    [hdn_id_estab] => 113^
    [hdn_nm_desc_funcionamento] => POSTO DE REVENDA DE GLP^
    [hdn_vl_desc_funcionamento] => 4,00^
    [hdn_nm_bloco_desc_funcionamento] => ^
    [hdn_id_desc_funcionamento_tmp] => 
    [txt_nm_desc_funcionamento_tmp] => 
    [txt_nm_bloco_desc_funcionamento_tmp] => 
    [txt_vl_desc_funcionamento_tmp] => 
    [txt_nm_usuario] => MARCELO VIANA DA SILVA AWK
    [cmb_ch_parecer] => D
    [txt_obs] => Observação
    [hdn_mtv_indeferimento] => D
    [txa_mtv_indeferimento] => asdf
    [hdn_controle] => 2
    [btn_incluir] => Alterar
)
*/
//  include ('../../templates/head.htm');
  ...
  $arquivo="alt_vist_funcionamento.php";
  ...
  $campos_obr= array('txt_nr_fone_empresa'=>"',txt_nr_cnpj_empresa,CNPJ da Empresa Solicitante,t'",
   'txt_nm_razao_social'=>"'txt_nm_razao_social,Razão Social,t'",
   ...
   'txt_id_edificacao'=>"'txt_id_edificacao,RE da Edificação,n'",
   ...
   'txt_id_cep'=>"'txt_id_cep,Número do CEP,t'",
   'txt_vl_area_construida'=>"'txt_vl_area_construida,Valor da Área total Construída,t'",
   'txt_vl_area_vistoriada'=>"'txt_vl_area_vistoriada,Valor da Área a ser Vistoriada,t'",
...
$campos_js=implode(",",$campos_obr);
  ...
  if ($campos_preenchidos) {
    ...
    if (strlen(str_replace("'","",$ID_CNPJ_EMPRESA))>13) {
      $CH_JURIDICA="'S'";
    } else {
      $CH_JURIDICA="'N'";
    }
...
// Consulta se a pessoa existe, se existe atualiza, se não existe insere
    ...
    $conn->query($query_pessoa);
    if ($conn->get_status()==false) $ERRO_TRANS.= "pessoa:\n".$conn->get_msg()."\n";

// Atualizar na tabela TBL_SOL_FUNC o campo CH_PROTOCOLADO='V'
    $query_solictacao = "UPDATE ".TBL_SOL_FUNC." SET CH_PROTOCOLADO='V' WHERE ".TBL_SOL_FUNC.".ID_SOLIC_FUNC=$ID_SOLIC_FUNC AND ".TBL_SOL_FUNC.".ID_CIDADE=$ID_CIDADE AND ".TBL_SOL_FUNC.".ID_TP_FUNC=$ID_TP_FUNC";
    $conn->query($query_solictacao);
    if ($conn->get_status()==false) $ERRO_TRANS.= "solicitacao:\n".$conn->get_msg()."\n";

// Atualizar na tabela TBL_PROT_FUNC os campos CH_VISTORIADO='S' e CH_STATUS_RETIRADA='V'
    ...
    if ($conn->get_status()==false) $ERRO_TRANS.= "protocolo:\n".$conn->get_msg()."\n";

// Caso não exita o ID_VISTORIA_FUNC inserir senão atualizar a tabela TBL_VISTORIA_FUNC 
    if ($ID_VISTORIA_FUNC==0) {
       $query_vist_funcionamento = "INSERT INTO ".TBL_VISTORIA_FUNC." (ID_VISTORIA_FUNC, ID_CIDADE, ID_PROT_FUNC, ID_CNPJ_EMPRESA, ID_CIDADE_EMPRESA, CH_PARECER, DE_OBSERVACOES, DT_VIST_FUNC, ID_USUARIO) VALUES ($ID_VISTORIA_FUNC, $ID_CIDADE,  $ID_PROT_FUNC, $ID_CNPJ_EMPRESA, $ID_CIDADE, $CH_PARCER, $DE_OBSERVACOES, $DT_VISTORIA,'$usuario')";
    } else {
       $query_vist_funcionamento = "UPDATE ".TBL_VISTORIA_FUNC." SET ID_CNPJ_EMPRESA=$ID_CNPJ_EMPRESA, ID_CIDADE_EMPRESA=$ID_CIDADE, CH_PARECER=$CH_PARCER, DE_OBSERVACOES=$DE_OBSERVACOES, DT_VIST_FUNC=$DT_VISTORIA, ID_PROT_FUNC=$ID_PROT_FUNC, ID_USUARIO='$usuario' WHERE ID_VISTORIA_FUNC= $ID_VISTORIA_FUNC AND ID_CIDADE=$ID_CIDADE";
    }
    if ($query_vist_funcionamento != "") {
      $res = $conn->query($query_vist_funcionamento);
      if ($ID_VISTORIA_FUNC==0) $ID_VISTORIA_FUNC = $conn->insert_id();
      if ($conn->get_status()==false) $ERRO_TRANS.= "Vistoria:\n".$conn->get_msg()."\n";
    }
...
//        $NR_PAVIMENTO=formataCampo($nr_pav[$i],"N","O");
        $NR_PAVIMENTO=0;
        ...
        //echo 'mostra parcial: '.$query_estab;
        $conn->query($query_estab);
        if ($conn->get_status()==false) {
...
<?
  if ($flg_pgto==0) {
?>
if (window.confirm("Pagamento Pendente, Deseja Pagar?")) {
    window.open("./modulos/funcionamento/baixa_funcionamento_pen.php?hdn_id_protocolo=<?=$ID_PROT_FUNC?>&hdn_id_cidade=<?=$ID_CIDADE?>","window_pgto","top=0,left=0,screenY=0,screenX=0,toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=no,width=780,height=500,innerwidth=780,innerheight=500");
  }
<?
  }
?>
  if (window.confirm("Deseja Imprimir Análise?")) {
    window.open("./modulos/funcionamento/rvist_funcionamento.php?txt_vist_habitese=<?=$ID_VISTORIA_FUNC?>&txt_id_cidade=<?=$ID_CIDADE?>","relanalise","top=5000,left=5000,screenY=5000,screenX=5000,toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=no,resizable=no,width=1,height=1,innerwidth=1,innerheight=1");
  }
//   window.location.href="alt_vist_funcionamento.php";
//--></script>
<?
    }
} else {
...
<script language="javascript" type="text/javascript">//<!--
    ...
    function consultaReg(campo1,campo2) {
      if ((campo1.value!="") && (campo2.value!="")) {
        window.open("./modulos/gerencial/cons_vist_funcionamento.php?gerencia="+1+"&campo1="+campo1.value+"&campo2="+campo2.value,"consulrot","top=5000,left=5000,screenY=5000,screenX=5000,toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=no,width=1,height=1,innerwidth=1,innerheight=1");
      }
    }
    ...
    function verpend(campo) {
      if (campo.value!="") {
        ...
        if (campo.value=="I") {
          ...
          document.frm_vist_funcionamento.txt_obs.value="Observação";
        }
...
/*      if (vfrm.cmb_nr_pavimento_desc_funcionamento_tmp.value=="") {
    ...
    function valida_sum_desc() {
      ...
      if ((parseFloat(vl_tot)-parseFloat(vl_vist))<0) {
        ...
        if ((parseFloat(vl_tot)!=(-9999999.99)) && (parseFloat(vl_vist)!=0)) {
          alert("Valor de Vistoria é MAIOR que a Área Construída");
        }
      } else {
    ...
    function envia(edificacao) {
      var frm_ed = document.frm_vist_funcionamento;
      if (edificacao==0) {
        window.open("../edificacoes/edificacao.php?hdn_id_solicitacao="+frm_ed.hdn_id_sol_funcionamento.value+"&hdn_id_cidade="+frm_ed.hdn_id_cidade.value+"&hdn_id_tipo_solicitacao="+frm_ed.hdn_id_tp_sol_funcionamento.value+"&form_padrao=frm_vist_funcionamento","cad_ed","top=0,left=0,screenY=0,screenX=0,toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=no,width=780,height=550,innerwidth=780,innerheight=550");
      } else {
        window.open("../edificacoes/edificacao.php?hdn_id_cidade="+frm_ed.hdn_id_cidade.value+"&hdn_id_edificacao="+edificacao+"&form_padrao=frm_vist_funcionamento","cad_ed","top=0,left=0,screenY=0,screenX=0,toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=no,width=780,height=550,innerwidth=780,innerheight=550")
							...
							<? } ?>
                              ...
                              <td width="83" align="center">Protocolo</td>
                                <td>
                                  ...
                                  <input type="text" name="txt_id_prot_funcionamento" class="campo_obr" size="15" maxlength="11" value="" title="Número do Protocolo" onblur="consultaReg(document.frm_vist_funcionamento.hdn_id_cidade,document.frm_vist_funcionamento.txt_id_prot_funcionamento)">
                                </td>
                              </td>
                            </tr>
                          </table>
                      ...
                      <tr>
                        ...
                        <fieldset>
                          ...
                          <table width="100%" cellspacing="0" border="0" cellpadding="2" align="center">
                            <tr>
                              <td>CNPJ</td>
                              <td><input type="text" name="txt_nr_cnpj_empresa" class="campo_obr" title="CNPJ/CPF do Solicitante" size="25" value="" onblur="cpfcnpj(this)"></td>
                              <td>Razão Social</td>
                              <td><input type="text" name="txt_nm_razao_social" class="campo_obr" title="Nome do Solicitante do Projeto" size="35" value="" maxlength="100"></td>
                            </tr>
                            <tr>
                              <td>Fantasia do Solicitante</td>
                              <td>
                                <input type="text" name="txt_nm_fantasia_empresa" class="campo" size="30" maxlength="100" title="Nome Fantasia da Empresa Solicitante">
                              </td>
                              <td>Contato</td>
                              <td>
                                <input type="text" name="txt_nm_contato" class="campo" size="40" maxlength="100" title="Nome da Pessoa de Contato da Empresa Solcitante">
                              </td>
                            </tr>
                            <tr>
                              <td>Fone</td>
                              <td><input type="text" name="txt_nr_fone_empresa" class="campo_obr" title="Fone do Solicitante" size="25" value=""></td>
                              <td>E-mail</td>
                              <td><input type="text" name="txt_de_email_empresa" class="campo_obr" title="E-mail do Solicitante do Projeto" size="35" value="" style="text-transform : none;" maxlength="100"></td>
                            </tr>
                          </table>
                        </fieldset>
                        </td>
                      ...
                      <tr>
                        ...
                        <fieldset>
                          ...
                          <table width="100%" cellspacing="0" border="0" cellpadding="2" align="center">
                            <tr>
                              <td>RE</td>
                              <td><input type="text" name="txt_id_edificacao" size="5" class="campo_obr" title="RE da Edificação" value="" maxlength="15" align="right" onkeypress="return validaTecla(this, event, 'n')" onkeyup="FormatNumero(this)" onblur="decimal(this,0)"></td>
                              <td>Nome</td>
                              <td><input type="text" name="txt_nm_edificacao" size="43" class="campo_obr" title="Nome da Edificação" value="" readOnly="true"></td>
                            </tr>
                            <tr>
                              <td>Tipo</td>
                              <td>
                                <input type="hidden" name="hdn_id_tp_logradouro" value="">
                                <input type="text" name="txt_nm_tp_logradouro" size="17" class="campo_obr" title="Tipo do Logradouro" value="" readOnly="true">
                              </td>
                              <td>Logradouro</td>
                              <td>
                                <input type="text" size="43" class="campo_obr" value="" name="txt_nm_logradouro" title="Nome do Logradouro" maxlength="100" readOnly="true">
                              </td>
                            </tr>
                            <tr>
                              <td>N°</td>
                              ...
                              <td>Bairro</td>
                              <td><input type="text" size="43" maxlength="50" value="" name="txt_nm_bairro" class="campo_obr" title="Nome do Bairro" readOnly="true"></td>
                            </tr>
                            <tr>
                              <td>CEP</td>
                              <td><input type="text" size="11" maxlength="10" name="txt_id_cep" value="" class="campo_obr" title="Número do CEP" onkeypress="return validaTecla(this, event, 'n')" onblur="CEP(this)" readOnly="true"></td>
                              <td>Cidade</td>
                              <td>
                                <input type="text" name="txt_nm_cidade" size="43" class="campo_obr" title="Cidade" value="" readOnly="true">
                              </td>
                            </tr>
                            <tr>
                              <td>Complemento</td>
                              <td><input type="text" name="txt_nm_complemento" class="campo" size="27" maxlength="100" value="" title="Complemento do Endereço da Edificação" readOnly="true"></td>
                              <td colspan="2">
                                <center>
                             ...
                             <tr>
                              ...
                              <td>
                              <input type="text" align="right" name="txt_vl_area_construida" class="campo_obr" title="Valor da Área Total Construida da Edificação" onkeypress="return validaTecla(this, event, 'n')" onkeyup="FormatNumero(this)" onblur="decimal(this,2)" readOnly="true" value=""><em>(m²)</em>
                              </td>
                              <td>Área Vistoriada</td>
                              <td colspan="3">
                              <input type="text" align="right" name="txt_vl_area_vistoriada" class="campo_obr" title="Valor da Área Vistoriada da Edificação" onkeypress="return validaTecla(this, event, 'n')" onkeyup="FormatNumero(this)" onblur="decimal(this,2)" readOnly="true" size="20"><em>(m²)</em>
                              </td>
                            </tr>
                          </table>
                        </fieldset>
...
// frm_at.cmb_nr_pavimento_desc_funcionamento_tmp.value="<?=$NR_PAVIMENTO?>";
