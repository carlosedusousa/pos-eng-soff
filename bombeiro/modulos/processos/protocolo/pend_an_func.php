<?
  ...
  $sql="SELECT ID_SOLICITACAO,".TBL_SOLICITACAO.".ID_CIDADE,ID_TIPO_SOLICITACAO, NM_EDIFICACOES_LX, DATE_FORMAT(DT_SOLICITACAO,'%d/%m/%Y') DT_SOLICITACAOS, (TO_DAYS('".date("Y-m-d")."') - TO_DAYS(DT_SOLICITACAO)) AS DIAS, NM_CIDADE FROM ".TBL_SOLICITACAO." JOIN ".TBL_CIDADE." USING(ID_CIDADE) WHERE ".TBL_SOLICITACAO.".CH_PROTOCOLADO='S' AND ".TBL_SOLICITACAO.".ID_TIPO_SOLICITACAO IN ('FS','FP') AND ".TBL_SOLICITACAO.".ID_CIDADE IN (SELECT ID_CIDADE FROM ".TBL_CIDADES_USR." WHERE ID_USUARIO='".$usuario."') ORDER BY DT_SOLICITACAO ASC";
...
<script language="javascript" type="text/javascript">//<!--
    ...
    function envia_pendencia(solicitacao,cidade,tipo) {
      ...
      var op = "pendenteFuncionamentoChamaPendenteProjeto";
      frm.op_menu.value = op;
      frm.submit();
    }
    function excluir(solicitacao,cidade,tipo) {
      if (window.confirm("Deseja Excluir a Solicitação?")) {
        var frm = document.frm_pendencia;
        frm.action="<?=$arquivo?>";
        frm.hdn_id_solicitacao.value=solicitacao;
        frm.hdn_id_cidade.value=cidade;
...
<form target="_self" enctype="multipart/form-data" method="post" name="frm_pendencia" action="index.php">
...
<input type="hidden" name="op_menu" value="">
            ...
            <table width="98%" cellspacing="1" border="0" cellpadding="5" align="center">
              <tr>
                <td>
                <fieldset>
...
?>
                          ...
                          <? if ($global_exclusao!="N") { ?>
                            <td align="center" onclick="excluir('<?=$pendente["ID_SOLICITACAO"]?>','<?=$pendente["ID_CIDADE"]?>','<?=$pendente["ID_TIPO_SOLICITACAO"]?>')"><img src="./imagens/b_drop.png" alt="" align="middle" border="0"></td>
                          <? } ?>
                        </tr>
<?
                        $cont++;
...
?>
                        <tr style="background-color : <?=COR_BARRA01?>;">
                          <td colspan="4" align="center"><b>Os campos em vermelho estão pendentes a mais de 20 dias</b></td>
                        </tr>
<? } ?>
                  </table>
                </fieldset>
