...
// Conectando ao BD
  $arquivo="solic_analise_func_local.php";
// Conectando ao BD
  ...
  if ($campos_preenchidos) {
	...
	"FROM ".TBL_SOLICITACAO." JOIN ".TBL_CIDADE." USING(ID_CIDADE) " .
		"WHERE ". 
			...
			TBL_SOLICITACAO.".ID_CIDADE IN (SELECT ID_CIDADE FROM ".TBL_CIDADES_USR." WHERE ID_USUARIO='".$usuario."') " .
	"ORDER BY CH_PROTOCOLADO DESC , DT_SOLICITACAO DESC";
	...
	if(!$encontrouSolicitacao || @$_POST[btn_enviar_solicitacao]) {
	    ...
	    if(!$query_desc_sub) {
	   		$NM_DESC_ANALISE_FUNC	= 'TOTAL';
			$NR_PAVIMENTOS_DESC		= $NR_PAVIMENTOS;
			$NM_BLOCO				= '';
			$VL_AREA_DESC_VISTORIAS	= $VL_AREA_CONTRUIDA;
            $query_desc_sub = " ($ID_CIDADE, $ID_SOLIC_ANALISE_FUNC, $ID_TIPO_SOLICITACAO, $ID_DESC_ANALISE_FUNC, '$NM_DESC_ANALISE_FUNC', $NR_PAVIMENTOS_DESC, '$NM_BLOCO',$VL_AREA_DESC_VISTORIAS)";
	    } 	
...
<script language="JavaScript" type="text/javascript">
  ...
  function verificarIgualdade() {
    ...
    if(frm.cbx_prop_igual_solic.checked) {
	  	frm.txt_nm_proprietario.value=frm.txt_nm_solicitante.value;
	  	frm.txt_nr_cnpjcpf_proprietario.value=frm.txt_nr_cnpjcpf_solicitante.value;
	  	frm.txt_fone_proprietario.value=frm.txt_fone_solicitante.value;
	  	frm.txt_de_email_proprietario.value=frm.txt_de_email_solicitante.value;
    } else {
	  	frm.txt_nm_proprietario.value='';
...
<? if(@$encontrouSolicitacao) { ?>
  ...
  <? } ?>	
    ...
    <tr>
      <td colspan="2" rowspan="1" style="vertical-align: top;">
        <fieldset>
          <legend>Solicitações para este CPF/CNPJ</legend>
          <table width="100%">
            <tr>
              <td colspan="2"><b><br>
          ...
          <legend>Proprietário</legend>
            <table>
              <tr>
                <td>&nbsp;</td>
                <td colspan="3">
                <input type="checkbox" name="cbx_prop_igual_solic" onclick="verificarIgualdade()"> Proprietário igual ao solicitante
              </tr>
              <tr>
                <td>Nome</td>
