...
<script language="javascript" type="text/javascript">//<!--
    ...
    function carregar_reg() {
      window.opener.document.frm_edificacao.hdn_id_crea_<?=trim($_GET["hdn_campo"])?>.value=document.frm_consulta_engenheiro.hdn_id_crea.value;
      window.opener.document.frm_edificacao.txt_nm_engenheiro_<?=trim($_GET["hdn_campo"])?>.value=document.frm_consulta_engenheiro.hdn_nm_engenheiro.value;
      window.close();
    }
          ...
          <form target="_self" method="get" name="frm_consulta_engenheiro" onreset="retorna(this)" onsubmit="return validaForm(this,'txt_pesq_engenheiro,Nome da Pesquisa,t')">
           <table width="95%" cellspacing="0" border="0" cellpadding="0" align="center">
              <tr>
                ...
                <td>
                  <input type="submit" name="btn_pesquisa" value="Pesquisar" class="botao" style="background-image : url('../../imagens/botao.gif');" title="Pesquisa Edificação">
                </td>
              </tr>
              <tr>
                <td colspan=4>
...
<?
  ...
  include('../../templates/btn_cons.htm');
